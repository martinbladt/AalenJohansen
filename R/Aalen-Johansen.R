...
#' @param k Number of nearest neighbours (alternative to kernel).
...
#' @export
#'
aalen_johansen <- function(data, x = NULL, a = NULL, k = NULL, p = NULL, collapse = FALSE, weights = NULL){
  ...
  if(is_unconditional){relevant_data <- 1:n}else{
    ...
    if(!is.null(k)){
      if(length(k) != 1 || k < 1 || k > length(X) || k != as.integer(k)) stop("k must be an integer between 1 and sample size")
      a <- 2 * sort.int(abs(X - x), partial = k)[k]
    }else if(is.null(a)){
      kde <- density(X, bw = "SJ")
      bw <- kde$bw
      a <- 2 * sqrt(3) * bw
