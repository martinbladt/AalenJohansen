...
We simulate $1,000$ independent and identically distributed realizations subject to independent right-censoring. Right-censoring follows the distribution $\text{Unif}(10,40)$.
...
Next, we fit the conditional Aalen--Johansen estimator for $u=1,5$.
...
For illustrative purposes, we plot the conditional state occupation probability $p_2$ using the conditional estimator (dashed) and the true model (full). This is done for both $u=1$ (in red) and $u=5$ (in blue).
